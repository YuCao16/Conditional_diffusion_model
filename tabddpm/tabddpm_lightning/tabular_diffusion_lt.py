...
parser.add_argument("--feat", type=int, default=256)
parser.add_argument("--save_dir", type=str, default=".data/tabular_diffusion_outputs")
parser.add_argument("--dataset_path", type=str, default="./higgs.csv")
...
class DDPM(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        _ts = _ts.type_as(x).type(torch.long)
...
def train_tabular(args: argparse.Namespace) -> None:
    ...
    for ep in range(n_epoch):
        ...
        with torch.no_grad():
            ...
            x_gen = ddpm.sample(n_sample, (1, 28)).reshape((n_sample, -1))
...
class LitTabDDPM(pl.LightningModule):
    def __init__(self, ddpm, learning_rate=1e-4, batch_size=256):
        super().__init__()
        self.ddpm = ddpm
    ...
    def train_dataloader(self, dataset_path = "./higgs.csv"):
        dataset = Dataset(dataset_path)
        return DataLoader(dataset, batch_size=self.batch_size, num_workers=12)
...
ddpm = DDPM(
    nn_model=ContextUnet(1, args.feat),
    betas=(1e-4, 0.02),
    n_T=args.timesteps,
    # device="cuda:0",
)
ddpm_model = LitTabDDPM(ddpm, 3.31e-5, args.batch_size)
# dataloader = ddpm_model.dataloader(args.dataset_path)

trainer = pl.Trainer(
    max_epochs=args.n_epoch, accelerator="gpu", callbacks=[RichProgressBar()]
)
trainer.fit(model=ddpm_model, train_dataloaders=dataloader)

# turn learning rate
# trainer = pl.Trainer(auto_lr_find=True)
# trainer.tune(model=ddpm_model, train_dataloaders=dataloader)

# turn batch_size
# trainer = pl.Trainer(auto_scale_batch_size="binsearch")
# datamodule = LitDataModule(batch_size=32)
# trainer.tune(
#     model=ddpm_model, train_dataloaders=datamodule
# )
